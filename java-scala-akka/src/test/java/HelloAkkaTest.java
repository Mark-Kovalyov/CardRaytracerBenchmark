...
public class HelloAkkaTest {
    ...
    public void testSetGreeter() {
        new JavaTestKit(system) {{
            TestActorRef<HelloAkkaJava.Greeter> greeter = TestActorRef.create(
                    system,
                    Props.create(HelloAkkaJava.Greeter.class), "greeter1");
    ...
    public void testGetGreeter() {
        new JavaTestKit(system) {{
            ...
            ActorRef greeter = system.actorOf(Props.create(HelloAkkaJava.Greeter.class), "greeter2");

            greeter.tell(new HelloAkkaJava.WhoToGreet("testkit"), getTestActor());
            greeter.tell(new HelloAkkaJava.Greet(), getTestActor());

            HelloAkkaJava.Greeting greeting = expectMsgClass(HelloAkkaJava.Greeting.class);
